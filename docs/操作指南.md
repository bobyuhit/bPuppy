...
### 5.5 电池电压 bpuppy_adc — ⚠ 已停用

**电池检测已停用** (2026-08): GPIO38 已改作舵机 (左前小腿), 且 ESP32-S3 可用 ADC 引脚 (GPIO1~20) 全部被占用, GPIO21 等非 ADC 引脚无法测量。`bpuppy_adc.init()` 当前打印警告并返回, `read_mv()` 返回 -1。
...
需要电池检测时: **复用摄像头 ADC1 脚** (如 GPIO4=ADC1_CH3 / GPIO5=ADC1_CH4), 不拍照时 deinit 摄像头 → init ADC → 软件乘系数换算。
