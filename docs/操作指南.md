...
```python
import bpuppy_imu
bpuppy_imu.init(0, 3, 21, 0x68)                  # 手动初始化 IMU
bpuppy_imu.calibrate(300)                        # 陀螺+加速度计校准 — 放平别动
...
**V3.0 (打样后)**: IMU SDA 改到 GPIO14, 电池分压改接 GPIO3=ADC1_CH2 (ADC1 在 BLE/WiFi 下可用) → 启用。硬件接线见 `docs/硬件连接.md` 电池电压测量节 (分压 47k/10k, 与原理图 R4/R5 一致)。

```python
import bpuppy_adc
bpuppy_adc.init()                      # 初始化 (V3.0: ADC1_CH2 GPIO3)
mv = bpuppy_adc.read_mv()              # ADC 引脚电压 (mV)
batt_v = mv * 5.7 / 1000               # 电池电压 (V), 分压换算 (47k/10k)
