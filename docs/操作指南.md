...
>
> **修复（固件定制）**：`components/mr9you__micropython-helper/mpy_startup.c` 中注释掉 `usb_init()` 调用，跳过 TinyUSB 初始化，GPIO19/20 回归普通 GPIO。**代价：USB-CDC 虚拟串口不可用**（REPL/烧录走 UART0=COM14，BLE 走射频，均不受影响）。若以后要用 USB 串口，恢复该调用即可（但 GPIO19/20 会再次被占）。
...
`frozen/main.py` 每次开机都会去板子上找 `/camera_on.py`：**文件在就执行，删掉就什么都不做**。
所以这个文件本身就是开关 —— 不用重编译固件，也不用敲指令，下次上电即生效。
...
- **开**：把 `mpy_modules/camera_on.py` 传到板子（ViperIDE 拖进去，或 `mpremote cp mpy_modules/camera_on.py :camera_on.py`）
- **关**：从板子删掉这个文件即可
- **和 `/main.py` 互不影响** —— KittenBlock 每次下载只重写 `/main.py`（它下载的用户程序），不会碰这个文件
- ⚠ 该文件必须是**纯 ASCII**（别写中文注释）：蓝牙传含中文的内容会丢字节。仓库里那份就是纯 ASCII 的
...
v = batt.read_batt_v()        # 已标定电池电压 (V)
print("电池电压: %.2f V" % v)
```

`mpy_modules/batt.py` 用实测标定点 (万用表 vs 程序显示) 最小二乘线性拟合 `实际 = a×显示 + b`, 校正 ADC 量程/分压偏差。上传: `mpremote cp mpy_modules/batt.py :batt.py`。换机/换电池后重新采集: `batt.add_cal_point(万用表V)` 追加标定点, `batt.verify()` 自查 (残差 ≤0.05V)。
