...
| 调用示例 | 默认值 | 说明 |
|------|--------|------|
| `bpuppy_motion.set_params(2.5, 70, 70)` | 2.5, 70, 70 | (speed, stride, height): 步频 / 步幅+方向 / 高度; speed=0停, stride正=前负=后, 默认 stride=70 |
...
`set_params` / `set_lift` 设置参数时自动校验，**超限/干涉则拒绝写入并保持原值**：

- **速度**：0~10
- **高度下限**：≥15mm（过低腿折叠干涉）
- **步幅上限**：约 106mm（前后脚干涉）
- **髋/膝角**：遍历足端实际摆动轨迹，任一点超限拒绝（含抬腿最高点）
- **抬腿过度**：`height - lift < 0` 拒绝
...
> 参数积木（速度/步幅/高度）会**立即生效**并更新变量，运动积木用最新值；参数超限（髋/膝角、抬腿过度）会被**校验拒绝**并保持原值。
